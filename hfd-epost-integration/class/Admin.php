...
//        add_filter('woocommerce_admin_order_actions', array($this, 'registerHfdButton'), 10, 2);
	...
	public function epostCancelEpostShipment(){
		...
		if( isset( $_POST['orderRef'] ) && !empty( $_POST['orderRef'] ) && wp_verify_nonce( $_POST['_ajax_nonce'], 'cancel_shipment' ) ){
    ...
    public function registerStyle()
    {
		...
		wp_localize_script( 'epost-admin-scr', 'hfd_admin_obj',
			array( 
				'ajaxurl'	=> admin_url( 'admin-ajax.php' ),
				'nonce' 	=> wp_create_nonce( 'cancel_shipment' )
			)
		);
    }
    ...
    public function saveSetting()
    {
        ...
        if (!empty($_POST)) {
            $settingKeys = array(
                ...
                'hfd_auto_sync_time',
                'hfd_sync_order_items',
                'hfd_take_varaible_sku'
            );
